...
Этот проект представляет собой Telegram-бота, который может обрабатывать изображения, отправленные пользователем, и выполнять с ними несколько операций: пикселизацию, преобразование в ASCII-арт, инверсию цветов, отражение по горизонтали или вертикали, создание тепловой карты, а также изменение размера изображения для использования в качестве стикера.
Бот также может отправлять случайные шутки и комплименты, симулировать подбрасывание монетки и сообщать пользователю
результат ("Орел" или "Решка"), а также предоставлять список доступных команд.
...
8. **Отправка случайных комплиментов**: Бот может отправлять пользователю случайные комплименты из предопределенного списка.

9. **Список команд**: Бот предоставляет список доступных команд, которые пользователь может использовать для взаимодействия с ботом.

10. **Подбрасывание монетки**: Бот может имитировать подбрасывание монетки и сообщать пользователю результат ("Орел" или "Решка").
...
**flip_coin(message)**: Симулирует подбрасывание монетки и сообщает пользователю результат ("Орел" или "Решка").
...
- **@bot.message_handler(commands=['random_compliment'])**: Реагирует на команду `/random_compliment`, отправляя случайный комплимент.
- **@bot.message_handler(commands=['flip_coin'])**: Реагирует на команду `/flip_coin`, имитируя подбрасывание монетки.
