...
выполнять с ними несколько операций: пикселизацию, преобразование в ASCII-арт и инверсию цветов. Бот использует
библиотеку `pyTelegramBotAPI` для взаимодействия с Telegram API и библиотеку `Pillow` для обработки изображений.
...
- **get_options_keyboard()**: Создает клавиатуру с кнопками для выбора пользователем, как обработать изображение: через
  пикселизацию, преобразование в ASCII-арт или инверсию цветов.
...
- **@bot.callback_query_handler(func=lambda call: True)**: Определяет действия в ответ на выбор пользователя (например,
  пикселизация, ASCII-арт или инверсия цветов) и вызывает соответствующую функцию обработки.
