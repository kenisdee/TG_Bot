...
или вертикали, а также создание тепловой карты. Бот использует библиотеку `pyTelegramBotAPI` для взаимодействия с
Telegram API и библиотеку `Pillow` для обработки изображений.
...
- **get_options_keyboard()**: Создает клавиатуру с кнопками для выбора пользователем, как обработать изображение: через
  пикселизацию, преобразование в ASCII-арт, инверсию цветов, отражение или создание тепловой карты.
...
- **@bot.callback_query_handler(func=lambda call: True)**: Определяет действия в ответ на выбор пользователя (например,
  пикселизация, ASCII-арт, инверсия цветов, отражение или создание тепловой карты) и вызывает соответствующую функцию
  обработки.
